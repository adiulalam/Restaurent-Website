...
class Builder_Contact
{
    ...
    public function contact_send() {
		...
		if ( $module_settings['field_email_active'] === 'yes' && ! is_email( $email ) ) {
		...
		$keys = array('public'=>$this->get_option('recapthca_public_key'),'private'=>$this->get_option('recapthca_private_key'));
		if ( $module_settings['field_captcha_active'] === 'yes' && $keys['public'] != '' && $keys['private'] != '') {
			if ( isset( $_POST['contact-recaptcha'] ) ) {
				$response = wp_remote_get("https://www.google.com/recaptcha/api/siteverify?secret=" . $keys['private'] . "&response=" . $_POST['contact-recaptcha']);
		...
		$sent=wp_mail($recipients[0], $subject, $message, $headerStr, $uploaded_files_path);
		...
		if ($sent) {
			...
			if ( ! $auto_respond_sent && ! empty( $module_settings['auto_respond'] ) && ! empty( $module_settings['auto_respond_message'] ) ) {
                ...
                add_filter('wp_mail_from_name', array($this, 'set_from_name_site'));
				wp_mail($email, $ar_subject, $ar_message, '');
                remove_filter('wp_mail_from_name', array($this, 'set_from_name_site'));
			}
		} else {
    ...
    public function set_from_name_site($name) {
        return get_bloginfo();
    }
