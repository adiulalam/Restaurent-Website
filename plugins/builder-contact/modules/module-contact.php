...
class TB_Contact_Module extends Themify_Builder_Component_Module {
	...
	protected function _visual_template() {
		...
		[ 'name', 'email', 'subject', 'message' ].forEach( function( v, i ) {
			if ( data["field_" + v + "_active"] === 'yes' ) {
				orders[ field_order["field_" + v + "_label" ] ] = v;
			}
		} );

		for ( i in field_extra ) {
			orders[ field_extra[i].order ] = i;
		}

		var isAnimated=data['layout_contact']==='animated-label';
		#>
				...
				<div class="builder-contact-fields tf_rel">
					...
					if( orders[i] === 'name' ) { #>
						...
						<label class="control-label"><span class="tb-label-span">{{data.field_name_label}} <# if( data.field_name_require ){ #></span><span class="required">*</span><# } #></label>
							...
							<input type="text" name="contact-name" placeholder="<# isAnimated===false && print(data.field_name_placeholder)#>" class="form-control" required />
						    </div>
					    </div>
					<# }
					else if( orders[i] == 'email' ){ #>
						...
						<label class="control-label"><span class="tb-label-span">{{data.field_email_label}}<# if( data.field_email_require ){ #></span><span class="required">*</span><# } #></label>
						    <div class="control-input">
							    <input type="text" name="contact-email" placeholder="<# isAnimated===false && print(data.field_email_placeholder)#>"  class="form-control" required />
						    </div>
					    </div>
					<# }
					else if( orders[i] == 'subject' ){ #>
						...
						<div class="control-input">
							<input type="text" name="contact-subject" placeholder="<# isAnimated===false && print(data.field_subject_placeholder)#>"  class="form-control" <# true === data.field_subject_require && print( 'required' ) #> />
						</div>
					    </div>
					<# }
					else if( orders[i] == 'message' ){ #>
					    <div class="builder-contact-field builder-contact-field-message builder-contact-textarea-field">
						    ...
						    <div class="control-input">
							    <textarea name="contact-message" placeholder="<# isAnimated===false && print(data.field_message_placeholder)#>" rows="8" cols="45" class="form-control" required></textarea>
						    </div>
					    </div>
					<# }
					else{
						index = orders[i];
						field = field_extra[ orders[i] ];
					    if(!field){
							continue;
					    }
					 #>
					<div class="builder-contact-field builder-contact-field-extra builder-contact-{{ field.type }}-field">
							<label class="control-label">{{{ field.label }}} <# if( field.required ){ #><span class="required">*</span><# } #></label>
							<div class="control-input">
							<# if( 'textarea' == field.type ){ #>
								<# field.value=field.value.replace(/\\\\n/g,'\n'); #>
                                <textarea name="field_extra_{{ index }}" placeholder="<# isAnimated===false && print(field.value)#>" rows="8" cols="45" class="form-control" <# true === field.required && print( 'required' ) #>></textarea>
							<# } else if( 'text' == field.type ){ #>
								<input type="text" name="field_extra_{{ index }}" placeholder="<# isAnimated===false && print(field.value)#>" class="form-control" <# true === field.required && print( 'required' ) #> />
							<# } else if( 'upload' == field.type ){ #>
								<input type="file" name="field_extra_{{ index }}" placeholder="<# isAnimated===false && print(field.value)#>" class="form-control" <# true === field.required && print( 'required' ) #> />
							<# } else if( 'tel' == field.type ){ #>
								<input type="tel" name="field_extra_{{ index }}" placeholder="<# isAnimated===false && print(field.value)#>" class="form-control" <# true === field.required && print( 'required' ) #> />
							...
							<# } else if( 'radio' == field.type ){
								_.each( field.value, function( value, k ){ #>
									<label>
										<input type="radio" name="field_extra_{{ index }}" value="{{ value }}" class="form-control" <# true === field.required && print( 'required' ) #> /> {{ value }}
									</label>
								<# });
							    } else if( 'select' == field.type ){ #>
								<select name="field_extra_{{ index }}" class="form-control" <# true === field.required && print( 'required' ) #>>
									<# _.each( field.value, function( value, k ){ #>
							...
							<# } else if( 'checkbox' == field.type ){
								 _.each( field.value, function( value, k ){ #>
									<label>
										<input type="checkbox" name="field_extra_{{ index }}[]" value="{{ value }}" class="form-control"/> {{ value }}
									</label>
								<# });
							} #>
							</div>
                    ...
                    <# data.field_send_align=undefined===data.field_send_align?'l':data.field_send_align; #>
					<div class="builder-contact-field builder-contact-field-send tf_clear tf_text{{ data.field_send_align[0] }}">
                        <button type="submit" class="btn btn-primary">{{{ data.field_send_label }}}</button>
					</div>
				</div>
			</form>
		</div>
